...
if ~exist(standardLAS2highresRAS, 'file') || optInputs(varargin, 'overwrite')
    unix_freesurfer(['tkregister2 --s ' subjid ' --mov ' standardLAS ' --targ ' highresLAS ' --fsl ' standardLAS2highresLAS_fsl ' --reg ' standardLAS2highresLAS_freesurf ' --noedit ']);
    unix_freesurfer(['tkregister2 --s ' subjid ' --mov ' highresLAS ' --targ ' highresRAS ' --reg ' highresLAS2highresRAS ' --noedit --regheader ']);
    unix_freesurfer(['mri_matrix_multiply -im ' standardLAS2highresLAS_freesurf ' -im ' highresLAS2highresRAS ' -om ' standardLAS2highresRAS]);
end