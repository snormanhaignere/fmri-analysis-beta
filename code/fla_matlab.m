...
% Modified on 4/7/2915 to use downsampled surface data
%
% Last Modified by Sam Norman-Haignere on 4/7/2015

fprintf('fla_matlab v2\n\n'); drawnow;
...
addpath('/mindhive/nklab/u/svnh/export_fig_v2');
...
for i = 1:length(conds)
    boxcar_or_impulse_function = zeros(nsmps,1);
    boxcar_or_impulse_function(condition_vector==i) = 1;
    
    x = conv(boxcar_or_impulse_function,h);
    model(:,i) = x(1:nsmps);
end
...
if optInputs(varargin, 'permute_condition_order')
    ...
    for q = 1:n_perms
        ...
        for i = 1:length(conds)
            boxcar_or_impulse_function = zeros(nsmps,1);
            boxcar_or_impulse_function(permuted_seq_upsampled(:,q)==i) = 1;
            x = conv(boxcar_or_impulse_function,h);
