...
elseif optInputs(varargin, 'detrend')
    funcvolume = [analysisdir 'preprocess/usub' num2str(us) '/' runtype '_r'   num2str(r) '/smooth' num2str(fwhm*100,'%.0f') 'mm_intnorm_detrend1'];
else
    funcvolume = [analysisdir 'preprocess/usub' num2str(us) '/' runtype '_r'   num2str(r) '/smooth' num2str(fwhm*100,'%.0f') 'mm_intnorm_hpfilt' num2str(params('hpcutoff'))];
end
...
if strcmp(stage,'film_gls')
    ...
    if optInputs(varargin, 'confound_regressors')
        confound_file = [stfdir 'usub' num2str(us) '/' runtype '_r' num2str(r) '_confound_regressors.stf'];
        unix_fsl(fsl_version, ['feat_model ' featdir '.feat/design ' confound_file]);
    else
