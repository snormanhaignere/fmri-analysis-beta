...
% 
% 2018-04-08: Changed how the first dicom file was read in
...
first_dicom_file = dir([dicoms_directory '/' '*-' num2str(seqid) '-1.dcm']);
if length(first_dicom_file)~= 1;
    error('Error: problem reading first dicom file');
end

% convert file
reference_dicom_file = [dicoms_directory '/' first_dicom_file.name]; %strrep(first_dicom_file.name, '-1-1', ['-' num2str(seqid) '-1'])];
