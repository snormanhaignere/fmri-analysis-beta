...
%
% Modified on 1/5/2014 to exclude grid points far away from all of the data points
%
...
for i = 1:2
    ...
    if strcmp(hemis{i},'lh')
        G.patch{i} = read_patch_SNH(patch_lh);
        G.roi_mask{i} = read_label_SNH(roi_lh_label);
        surf_data{i} = MRIread(surface_lh);
    elseif strcmp(hemis{i}, 'rh')
        G.patch{i} = read_patch_SNH(patch_rh);
        G.roi_mask{i} = read_label_SNH(roi_rh_label);
    ...
    try
        ...
        if isvector(surf_data{i}.vol)
            vr = surf_data{i}.vol(G.vi{i}+1);
        else
    ...
    % minimum distance of each grid point to all of the surface vertices
    min_dist = nan(size(G.grid_x{i}));
    for q = 1:numel(G.grid_x{i})
        x = sqrt((G.grid_x{i}(q) - G.vras{i}(:,1)).^2 + (G.grid_y{i}(q) - G.vras{i}(:,2)).^2);
        min_dist(q) = min(x);
    end
    ...
    for q = 1:M
        ...
        try
            ...
            vr_gridded(min_dist > grid_spacing_mm*2) = NaN;
